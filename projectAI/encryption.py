...
def run_gui():
    ...
    sender1_chat_window.grid(row=0, column=0, padx=10, pady=10, columnspan=3)
    ...
    # File attachment button for Sender1
    def attach_file_sender1():
        file_path = filedialog.askopenfilename()
        if file_path:
            sender1_entry.insert(tk.END, f"File: {file_path}")

    attach_button_sender1 = tk.Button(sender1_window, text="Attach File", command=attach_file_sender1)
    attach_button_sender1.grid(row=1, column=1, padx=10, pady=10)

    # Send Button for Sender1
    ...
    send_button_sender1.grid(row=1, column=2, padx=10, pady=10)
    ...
    sender2_chat_window.grid(row=0, column=0, padx=10, pady=10, columnspan=3)
    ...
    # File attachment button for Sender2
    def attach_file_sender2():
        file_path = filedialog.askopenfilename()
        if file_path:
            sender2_entry.insert(tk.END, f"File: {file_path}")

    attach_button_sender2 = tk.Button(sender2_window, text="Attach File", command=attach_file_sender2)
    attach_button_sender2.grid(row=1, column=1, padx=10, pady=10)

    # Send Button for Sender2
    ...
    send_button_sender2.grid(row=1, column=2, padx=10, pady=10)
    ...
    # Dropdown menu for admin to choose message view option
    options = ["All Messages", "Encrypted Messages", "HMAC Messages"]
    selected_option = tk.StringVar(admin_window)
    selected_option.set(options[0])  # Default to All Messages
    option_menu = tk.OptionMenu(admin_window, selected_option, *options)
    option_menu.grid(row=1, column=0, padx=10, pady=10)

    # Display original, encrypted message, and HMAC status based on selected option
    def monitor_message(message, option):
        encrypted_message = encrypt_message(message)
        message_hmac = compute_hmac(message)

        if option == "All Messages":
        ...
        elif option == "Encrypted Messages":
            admin_chat_window.config(state=tk.NORMAL)
            admin_chat_window.insert(tk.END, f"Encrypted Message: {encrypted_message}\n")
            admin_chat_window.insert(tk.END, "---------------------------------------\n")
            admin_chat_window.config(state=tk.DISABLED)

        elif option == "HMAC Messages":
            admin_chat_window.config(state=tk.NORMAL)
            admin_chat_window.insert(tk.END, f"HMAC of Message: {message_hmac}\n")
            admin_chat_window.insert(tk.END, "---------------------------------------\n")
            admin_chat_window.config(state=tk.DISABLED)
    ...
    def send_message_sender1(entry, sender1_chat_window, sender2_chat_window, sender, position):
        ...
        if message:
            ...
            monitor_message(message, selected_option.get())
            ...
            if "File: " in message:
                file_path = message.split("File: ")[1]
                encrypted_file = encrypt_file(file_path)
                # Simulate sending the encrypted file
                sender2_chat_window.config(state=tk.NORMAL)
                sender2_chat_window.insert(tk.END, f"{sender}: [File Attached]\n")
                sender2_chat_window.config(state=tk.DISABLED)
    ...
    def send_message_sender2(entry, sender2_chat_window, sender1_chat_window, sender, position):
        ...
        if message:
            ...
            if "File: " in message:
                ...
                # Simulate sending the encrypted file
                sender1_chat_window.config(state=tk.NORMAL)
                sender1_chat_window.insert(tk.END, f"{sender}: [File Attached]\n")
                sender1_chat_window.config(state=tk.DISABLED)
